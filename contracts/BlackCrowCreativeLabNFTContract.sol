...
error BlackCrowCreativeLabNFTContract_MaxSupplyReached();
error BlackCrowCreativeLabNFTContract_ValueNotEqualPrice();
error BlackCrowCreativeLabNFTContract_WrongAvenueForThisTransaction(); 


// @custom:security-contact tracy@blackcrowcreativelab.com
contract BlackCrowCreativeLabNFTContract is 
ERC721, 
ERC721Enumerable, 
ERC721URIStorage, 
ERC721Burnable, 
...
event FundsDistributed(address owner, uint amount);
    ...
    constructor(
        ...
        ERC721("BlackCrowCreativeLabNFTContract", "BCCL")
        Ownable(msg.sender)
    {
    ...
    receive() external payable { 
        revert BlackCrowCreativeLabNFTContract_WrongAvenueForThisTransaction();
    }
    ...
    function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert BlackCrowCreativeLabNFTContract_MaxSupplyReached();
        }

        //make sure the right amount of money
        if (msg.value != i_mint_price) {
            revert BlackCrowCreativeLabNFTContract_ValueNotEqualPrice();
        }
        _tokenIdCounter++;
    ...
    function contractURI() public view returns (string memory) {
        ...
        function setRoyalty( address _receiver, uint96 feeNumerator)
            public onlyOwner {
